...
const getPasswordStrength = (password: string): PasswordStrength => {
  ...
  const passed = PASSWORD_TESTS.filter(t => t(password)).length;
...
const SignUpEmailScreen = ({navigation}) => {
  ...
  const [validateFields, {isLoading: isValidating}] = useValidateFieldsMutation();
  const [sendVerificationCode, {isLoading: isSendingCode}] =
    useSendVerificationCodeMutation();
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (!validationResult.isValid) {
        ...
        setMissingFields(Object.keys(errors));
        ...
        setFieldErrors(prev => ({
          ...
          general:
            sendError === 'TOO_MANY_REQUESTS'
              ? t('auth.emailVerification.tooManyRequests')
              : t('auth.emailVerification.sendFailed'),
        }));
        return;
      }
  ...
  const isSubmitting = isValidating || isSendingCode;
            ...
            return (
              <View key={index} style={styles.ruleRow}>
                <MaterialCommunityIcons
                  ...
                  size={14}
                  color={passed ? '#10B981' : GlobalColors.Onboarding.textMuted}
                />
                <Text
